...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            var spreadBetweenShots = shotgunSpread / shotgunBullets;
            var halfOfBullets = shotgunBullets / 2;
            for (int i = 0; i < shotgunBullets; i++)
            {
                SpawnBullet(director.position, director.rotation * Quaternion.Euler(0, 0, (i-halfOfBullets) * spreadBetweenShots));
            }
        }
        else
